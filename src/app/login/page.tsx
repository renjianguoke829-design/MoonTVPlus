...
function LoginPageClient() {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (rememberedCredentials) {
        try {
          ...
          if (credentials.username && shouldAsk) setUsername(credentials.username);
  ...
  useEffect(() => {
    if (!siteConfig?.LoginRequireTurnstile || !siteConfig?.TurnstileSiteKey) return;
  ...
  useEffect(() => {
    if (!turnstileLoaded || !siteConfig?.TurnstileSiteKey) return;
    ...
    try {
      ...
      if (res.ok) {
        if (rememberPassword) {
          ...
          if (shouldAskUsername && username) credentials.username = username;
    ...
    >
      ...
      <div className='relative z-10 w-full max-w-md rounded-3xl bg-gradient-to-b from-white/90 via-white/70 to-white/40 dark:from-zinc-900/90 dark:via-zinc-900/70 dark:to-zinc-900/40 shadow-2xl p-10 dark:border dark:border-zinc-800'>
        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <div>
            ...
            {shouldAskUsername && (
              <div className='flex justify-end mt-2'>
                <a href='/forgot-password' className='text-xs text-green-600 dark:text-green-400 hover:text-green-700 hover:underline transition-colors'>
                  忘记密码？
                </a>
              </div>
            )}
          ...
          <button
            type='submit'
            disabled={!password || loading || (shouldAskUsername && !username) || (siteConfig?.LoginRequireTurnstile && !turnstileToken)}
